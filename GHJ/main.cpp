...
void print(vector<int> &join_res, Disk* disk) {
...
int main(int argc, char* argv[]) {
	...
	Disk disk = Disk();
	Mem mem = Mem();
	pair<int, int> left_rel = disk.read_data(argv[1]);
	pair<int, int> right_rel = disk.read_data(argv[2]);
	
	/* Grace Hash Join Partition Phase */
	vector<Bucket> res = partition(&disk, &mem, left_rel, right_rel);

	/* Grace Hash Join Probe Phase */
	vector<int> join_res = probe(&disk, &mem, res);
