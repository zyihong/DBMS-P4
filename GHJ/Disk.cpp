...
void Disk::print(int id){
	this->pages[id]->print();
}

void Disk::print(){
	for(unsigned i = 0; i < this->pages.size(); i++){
		if(this->pages[i]){
...
pair<int, int> Disk::read_data(const char* file_name) {
	...
	int end_page_id = int(this->pages.size());
	raw_data_file.close();
